...
void drawDailyQuestsScreen() {
  ...
  gfx->printf("STREAK: %d", quest_data.streak_days);
  ...
  for (int i = visible_start; i < visible_end; i++) {
    ...
    gfx->fillRect(30, barY, barW, 12, RGB565(20, 22, 30));
    int fillW = (q.progress * barW) / q.target;
    if (fillW > barW) fillW = barW;
    gfx->fillRect(30, barY, fillW, 12, q.completed ? RGB565(0, 200, 80) : theme->primary);
    
    // Progress text
    gfx->setTextSize(1);
    gfx->setTextColor(RGB565(150, 155, 170));
    gfx->setCursor(LCD_WIDTH - 60, barY + 2);
  ...
  gfx->setTextSize(2);
  gfx->setCursor(LCD_WIDTH/2 - 30, LCD_HEIGHT - 30);
  gfx->print("< SWIPE >");
